...
> Parte da skill **schematize-ruby**. **É o DONO do handoff de contexto**: o §34.1 do `entrega.md` **da base** (`schematize-engineering`) é o *gatilho* (quando parar e arquivar); o *como* — formato dos dois MDs, hooks, `/compact`, o que entra em cada seção — é aqui, e só aqui. Implementa também a §28 (archive). Conteúdo específico de ferramenta (Claude Code / VS Code) — o princípio normativo está no `entrega.md` **da base** (`schematize-engineering` -> `references/entrega.md` §34.1) — o recorte local deixou de repeti-lo quando virou ponteiro.
...
## Comando `/ruby-cc` (context compact) — handoff manual sob demanda

No Claude Code, comandos e skills são unificados: ambos criam um `/nome`. Há dois formatos — `.claude/commands/<nome>.md` (legado, mais simples) e `.claude/skills/<nome>/SKILL.md` (recomendado; suporta `/nome` mais invocação autônoma, e aceita o campo `command:` no frontmatter). Uma pasta de skill = um comando, então um `/ruby-cc` próprio é um arquivo de comando dedicado.

Bundlado em `assets/commands/ruby-cc.md`. Instalação:

1. Copie `assets/commands/ruby-cc.md` para `.claude/commands/ruby-cc.md` na raiz do projeto.
2. Digite `/ruby-cc` na sessão (ou `/ruby-cc <foco>` pra direcionar o resumo, via `$ARGUMENTS`).

O `/ruby-cc` gera dois MDs em `<projeto>_archive/context/`:
...
E então roda `/compact` com foco. Diferença pro automático: o StatusLine/PreCompact é o gatilho **automático** no limite; o `/ruby-cc` é o handoff **manual** que você dispara quando quiser fechar um marco. Os dois gravam no mesmo `<projeto>_archive/context/`.

Alternativa em formato skill (auto-invocável além do `/ruby-cc`): criar `.claude/skills/ruby-cc/SKILL.md` com frontmatter `name`, `description` e `command: /ruby-cc`, e o mesmo corpo do `cc.md`.
