...
- **Senha por padrão, opcional por escolha:** o usuário **cria senha no cadastro** (padrão
  ...
  viver de passkey/OTP/app.
  > **`has_secure_password`/bcrypt NÃO é opção viva para senha nova.** A casa é **argon2id-only**
  > (parâmetros mínimos na base: `schematize-engineering`, seção 2.1 do `references/iam.md` dela). O `bcrypt` que
  > o Rails traz por default é **legado a migrar**: mantenha a verificação para quem já tem hash
  > bcrypt e faça **re-hash preguiçoso para argon2id no próximo login bem-sucedido**, com o
  > algoritmo gravado junto do hash. Tratar bcrypt como alternativa aceitável (e só aqui) era
  > deriva por cópia — a `schematize-node` já o trata como dívida.
  > **No login, `authenticate_by`, nunca `find_by` + `authenticate`** (Rails 7.1+): o login "óbvio"
  > **não faz o trabalho de hash quando o e-mail não existe** e responde muito mais rápido nesse
  > caminho — **enumeração de usuário por timing**, medível de fora. O `authenticate_by` gasta o
  > mesmo tempo nos dois casos; a resposta é genérica nos dois (`references/seguranca.md`).
  ...
  > o valor medido no ADR do serviço; o default da lib normalmente é o mais fraco. Na gem `argon2`, `m_cost`/`t_cost` explícitos; o default da gem fica abaixo do piso.
...
class ExternalRecipientGuard
  ...
  # ele não publicar MX — propriedade fora do nosso controle. Piso de rota nula é DNS
  # que nós controlamos ou TLD que a IETF reservou.
  DEFAULT_TEST_DOMAINS = %w[test.acme.com test invalid example].freeze
  ...
  def delivering_email(message)
    # A CHECAGEM DE DESTINATÁRIO é a que só vale fora de produção — produção entrega
    # de verdade. O CAP não: ele vale em TODOS os ambientes. O cap não é uma das
    # camadas de sandbox, é um **freio** contra laço em massa (ADR-0004), e um laço
    # em massa em produção é pior, não melhor. Um `return if production?` no topo
    # tornava o bloco do cap inalcançável em prd — e contradizia o CLAUDE.md desta
    # skill, que enuncia o cap sem qualificador de ambiente.
    unless production?
  ...
  #
  # O `split("@").last` SOZINHO é uma armadilha: `"invalid".split("@").last` devolve
  # `"invalid"` — a string INTEIRA vira "domínio" e casa com a allowlist. Sem a
  # checagem do `@`, `"test"`, `"invalid"` e `"example"` passavam pelo guard sendo
  # endereços malformados, e `"@test"` passava com local part vazia.
  def test_recipient?(address)
    addr = address.to_s.downcase
    at = addr.rindex("@")
    return false if at.nil? || at.zero?   # sem "@", ou local part vazia: não é endereço

    domain = addr[(at + 1)..].to_s
