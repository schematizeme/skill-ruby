...
> `references/anti-padroes.md`. Leia lá primeiro; aqui fica **só o que muda em Ruby**.
>
...
14. **`Access-Control-Allow-Origin: *` em rota autenticada** (pior com `credentials: true`) no `rack-cors`.
    → Allowlist explícita de origens no `config/initializers/cors.rb` (hardening; ver a `schematize-qa`, `references/categorias.md` §§5 e 10).

15. **Endpoint de debug/admin (Sidekiq Web UI, `/rails/info`, console web) sem auth, ou bind em `0.0.0.0`** expondo porta interna.
    → Bind restrito, auth obrigatória, montar Sidekiq/admin atrás de `authenticate` no routes; `web-console` só em dev (ver a `schematize-qa`, `references/categorias.md` §§5 e 10).
