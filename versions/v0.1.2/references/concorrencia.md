...
**Sidekiq** processa jobs com **concorrência por threads** dentro de um processo worker.

**MUST**
- Jobs em Sidekiq são **I/O-bound e idempotentes** (§19): o job pode rodar 2x (retry,
  crash entre efeito e ack) — reexecutar não pode duplicar cobrança/estado.
- **Pool de conexões do DB ≥ `concurrency` do Sidekiq** — senão as threads brigam por
  conexão e travam (mesmo piso do Puma, C3).
- **Retry com limite e backoff**; job que estoura o limite vai pra dead set e
  **alerta** — não some (liga com `observabilidade.md`).

**SHOULD**
- CPU-bound pesado **não** vai num Sidekiq de alta concorrência (as threads se
...
| I/O-bound assíncrono (fora do request) | **Sidekiq** | idempotente, retry+backoff, pool DB ≥ concurrency |
