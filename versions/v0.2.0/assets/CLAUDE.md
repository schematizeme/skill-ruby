...
17. **Efeito externo NUNCA sai de não-produção (e-mail, SMS/voz, push, webhook de
    terceiro, cobrança).** Fora de `production` **nada chega em ninguém** — por
    construção, não por lembrança. **(a)** `config.action_mailer.delivery_method`
    é **`:test`** no test, **`:letter_opener`/Mailpit SMTP** em dev e staging, **real
    só em `production`** (`ENV.fetch` sem default: sem credencial o boot **falha**,
    não sinka em silêncio). **(b)** O guard é um **INTERCEPTOR**
    (`ActionMailer::Base.register_interceptor`), registrado **sem condição de
    ambiente** — pega **TODO mailer, inclusive os que alguém escrever depois**;
    destinatário fora do domínio de teste com `Rails.env != "production"`
    **levanta erro** e aborta a entrega (nunca warning/no-op). **(c) Cap por
    execução** (`MAIL_MAX_PER_RUN`, default 50) que **aborta**, contador com
    `Mutex`; **fail-closed**: só a string exata `"production"` entrega de verdade.
    **(d)** Endereço sintético (spec, FactoryBot `sequence`, `db/seeds.rb`, rake,
    `simulated`) só como `<papel>+<run-id>-<n>@test.<domain>` — **rota nula**
    (null MX RFC 7505 + SPF `-all` + DMARC `p=reject`) ou TLD reservado
    (`.test`/`.invalid`/`.example`); **VETADO** `@gmail.com`/`@hotmail.com`,
    domínio de terceiro, e-mail de pessoa real (inclusive o seu) e o domínio de
    **produção**. Chave **sandbox** fora de prd; `WebMock.disable_net_connect!`
    ligado. Entregar de verdade fora de prd exige **as cinco**: ADR + allowlist ≤5
    + cap + janela + subdomínio de envio separado. **Por quê:** bounce/complaint em
    massa **queima IP e domínio**, derruba o transacional de **produção** (inclusive
