---
description: schematize-ruby — audita/scaffolda o <projeto>_ops (interface única) e verifica o fluxo de ambientes, a instalação paralela (nproc) e a independência dos serviços
...
- O ops provisiona em **`/<app>/`** e **clona os repos dentro** (`/<app>/<app>_<ctx>`, ex. `/payle/payle_core_rb`). Repo fora de `/<app>/` é achado.
- **`/<app>/.env` é o seeder global** — fonte única de config; nenhum serviço com config à parte fora do seed.
- **Redeploy é destrutivo na app:** `ops redeploy` apaga a implantação anterior, faz `bundle install --deployment --frozen` e recria um clone **zerado** só com o seed (sem patch in-place, sem drift). Prove idempotência: dois redeploys seguidos → mesmo estado.
- **Dados NUNCA no destrutivo:** banco/volumes/uploads preservados (migration reversível — `rails db:migrate`/`db:rollback`). Apagar dado é `ops reset`, **gated a dev/hml**, com confirmação — nunca prd.
...
- **Um systemd unit hardened por serviço** — o app Ruby (Puma) e o **Sidekiq** são units separados (`User=`, `NoNewPrivileges`, `ProtectSystem=strict`, `ProtectHome`, `PrivateTmp`, `ReadWritePaths` mínimo).
...
`bootstrap` · `install`/`up` (Puma + Sidekiq) · `update` · `config` · `migrate` (reversível) · `health`/`doctor` · `rollback` · `logs`/`troubleshoot` · `test` (`bundle exec rspec`).
...
- Meta de completude: **`ops install` provisiona um servidor do zero** sozinho (Ruby via rbenv/asdf pinado, gems, DB, Redis).
