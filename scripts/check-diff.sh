...
# auth, coerção de tipo, etc.) fica pro /ruby-review com leitura humana/da IA.
...
# 2.1) §37 — macaquices específicas de Ruby
scan '\.(where|find_by_sql|order|group|having|pluck)\(\s*["'\''][^"'\'']*#\{' 'interpolação de string em query ActiveRecord — use placeholder (?/hash) (§10/§37)'
scan '(reset|token|secret|session|nonce|csrf)[a-z_]*\s*=\s*rand\b|\brand\([^)]*\).*(token|secret|session|nonce)' 'rand em contexto de segredo — use SecureRandom (§14/§37)'
scan 'YAML\.load\(|Marshal\.load\(|Oj\.load\(' 'desserialização insegura — use YAML.safe_load/allowlist (§37)'
scan 'params\.permit!|params\.to_unsafe_h|params\.to_unsafe_hash' 'mass-assignment sem strong params (permit!/to_unsafe_h) (§37)'
scan 'rescue\s+nil\b|rescue\s*=>\s*\w+\s*;?\s*end|rescue\s*;\s*end' 'erro engolido (rescue nil / rescue vazio) (§37)'
scan '\balg\b\s*[:=]\s*["'\'']?none' 'JWT alg=none — allowlist de algoritmos (§37)'
